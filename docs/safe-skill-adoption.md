...
in four of them (5, 6, 9, 10); it documents and connects the rest.
...
3. [**Scan**](#scanning-and-manual-review) it — scanners produce evidence, not certification.
...
8. [**Quarantine**](#quarantine-the-update) the update; the active copy is untouched.
9. [**Diff**](#staging-and-comparing-the-update) the two trees — `sigildex diff`.
10. [**Re-approve**](#re-approve) — a human reads the diff, `lock` records the new approval, CI keeps the decision human.
...
It is not a certificate that the skill is safe, not proof that anyone
...
what was approved? Everything else in this guide is what makes that answer mean
something. Trust comes from where the records live and who can change them: a
protected branch, code-owner review, a required status check.
...
from version 2.90.0; the Vercel Skills CLI and skills.sh), from publisher
repositories, and from your own organization.

Popularity is the weakest signal. Before you spend review effort, capture:
...
- **Provenance** — the repository, the subdirectory, and the exact commit or
  tag. You will need these three values again for update checks.
...
- It sits outside every active skills directory and every path your agent
  harness scans.
...
scanner below says so in its own documentation. Run more than one — they
...
already run — exactly what quarantine exists to prevent.
...
- `--out` — required; the parent directory must exist; the filename must be
  `<approval-id>.lock.json`; the record may not sit inside the measured
...
- `--source-*` — optional; record an unverified `declared_source` hint that sits
...
Copy the approved artifact from quarantine into the place your agent loads it,
...
- **Installer-pinned** — stronger pinning, but `gh skill update` skips anything
  ...
  release watcher so that "pinned" never quietly becomes "unwatched".
...
- **Scheduled** — for repositories. A workflow in your own account runs a
  ...
  has moved. It never merges, never replaces the trusted skill, and never
  records an approval: its only job is to tell a human there is something to
  look at.
...
installation — and prove the active copy was untouched by running the same check
before and after:
...
Those are settings, not cryptography — anyone who can change branch protection can bypass
...
2. **Copy to review staging.** Review the copy, so the active one stays
...
3. Restart the agent harness. Many harnesses read skills at startup or cache
   them, so deleting files may leave what is resident loaded. Assume a restart
   is required unless your harness documents otherwise.
...
5. Treat every credential the skill could reach as exposed and rotate it. The
   record lists which files were present (paths, sizes, digests, modes) but not
...
Git, so a previous approved state is a previous commit. Restore both together,
then verify:
