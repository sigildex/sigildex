...
The plan to rebuild the golden set was locked with an accepted human workload of roughly 230 to 300 labels and a pre-declared threshold for unsafe exposure. The machine side went well: a candidate pool of 63 queries and 3,194 query-skill pairs, bracketed against the 84,769-row corpus, drafted with zero errors for $2.12; the whole relabeling step ran to $4.24 of model spend against the $25 ceiling.

The human workload is what exploded. An adversarial re-review of the first adjudication packet returned a change-request verdict with two blocking findings, both accepted: the proposed slate had quietly narrowed the labeling populations the locked plan specified, and had dropped an entire channel — manually seeded retrieval misses — that existed precisely to measure what retrieval never surfaced. Rather than reinterpret the locked plan, we treated the fix as an explicit contract amendment and re-ratified it.
...
Three process findings transfer beyond this project. Fail-closed error contracts and typed exit codes should be the default for anything an agent drives unattended, because an agent cannot infer intent from a bare non-zero exit. Identity-bracketed capture plus digest-bound blind sampling is a general recipe for benchmark integrity, cheap to build before you need it. And in the case we measured, adversarial review across two model families found roughly 95% disjoint sets of issues on the same code.
