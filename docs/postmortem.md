# Postmortem: I built an index of agent skills. I'm shipping without one.
...
I had two goals going in. Build something useful for agents. And get more hands-on experience building with agents, since my personal workflows for that were still forming. The first goal ended up smaller than the pitch. The second one paid off more than I expected. Here is how it went.
...
The first index was built within a couple of days: 78,394 skills, 37,604 from GitHub and 40,790 from ClawHub. Payments were gated on every endpoint a few days after that. That part took a week. Everything after it took five months.
...
So at the end of April I decided to focus on those agents. Days later my ClawHub sync job started timing out against my own database: 40-minute runs, two days straight, then 711 statement timeouts from write contention. I froze that source at its last good sync. The public API defaulted to GitHub-only from then on. Roughly 80% of the segmentation design I'd been planning fell away, and I kept the crawler code in case I ever wanted it back. I never did.

## Inspection and verification were the interesting part

Safety was a launch blocker from the start: an index that recommends a malicious skill is worse than no index. The first bulk pass ran a three-layer scoring pipeline over 88,903 skills for about $100 in model spend. Across the ~94K skills scored, 85.6% came back safe, 5.4% caution, 1.8% warning, and 2.1% flagged dangerous by the pipeline (those four shares don't account for the whole corpus in my records).
...
I fired the rebuild four times in early June and eleven more times between mid-June and mid-July. Most attempts died on a new defect, usually deeper than the last. The ones worth telling:
...
- Two rows out of 85,137 came back 403 at insert: their content pattern-matched a web-application-firewall rule at the database's edge, which is what such rules are for. The dry-run probe that should have caught this had been silently dead since the day it shipped. It relied on a transaction-rollback header the REST layer doesn't honor. All 15,055 probe attempts had been no-ops.
...
Eventually I salvaged a complete build: 84,769 skills, fully scored and embedded, every quality gate green until the retrieval evaluation. Then the retrieval eval collapsed. NDCG@10 went from 0.633 to 0.122. The index wasn't broken. The instrument was. The golden set dated from May, against the mixed corpus. 62 of its 232 relevant labels pointed at ClawHub rows the GitHub-only corpus by definition didn't contain. The other 170 were joined on row IDs a rebuild doesn't preserve; only 44 could be recovered by that key. Rebuilding the golden set properly came out at 986 human labeling decisions. I had done 17 when I stopped.

## The payments experiment I didn't get to

The part I most wanted to build going in was the merchant side: a service with no signup, no API key and no invoice, where an agent pays per call and the wallet is the identity. That shipped. x402 on Base with USDC through the Coinbase facilitator, $0.002 per query, gated on REST and MCP, with a Sign-In-With-X lane so a wallet could hold a higher free tier without an account. In early July I ran the live test: three paid calls, $0.006, all settled on-chain.

What I didn't do was innovate on it. I'd wanted to find out what a truly headless merchant looks like once a second rail (Stripe's Machine Payments Protocol) sits next to x402, and I'd done the research: support both, and neither one solves agent identity, metering or reconciliation for you. Then reality. Real x402 volume was still small in mid-2026 and I couldn't tell how much of it was organic, so "pay per call" couldn't be the reason anyone adopted this. At my price point the facilitator's per-transaction fee ate about half of each call. And the crawl and rebuild consumed nearly all the engineering attention. Payment stayed a rail. It never became a product. I'm picking that thread up in my next project.
...
In July the honest plan was: launch as-is, give it six weeks, let adoption decide, accept either answer. In August I drafted the exit instead: pivot to open source and stop. I asked for a hard second opinion on it. The review pushed back on the plan as written, because I was foreclosing the market test without data, and I think it was right about that. It also conceded the strongest case for stopping. Three plans in a row to finish the index had come back "don't ship", and by then that's evidence about the real cost to finish, not about the reviewers. "Just finish the labeling" hides the recurring term underneath it: crawl operations, freshness upkeep, hosting, keeping public claims true as the corpus drifts, on call for all of it, indefinitely, alone. And it warned that a wind-down built on optimistic sizing reproduces the exact failure it exists to escape. So I timeboxed it.

In mid-August I made the call. Two weeks, bounded, no index to maintain: ship the piece that survives without one, write this down, and turn the hosted service off.

To be plain: I never ran the market test. I never publicised it, so there is no meaningful traffic, usage or retention data. That is a gap in the evidence, not a finding about demand.
...
**Decouple the market test from index completeness.** Collecting adoption signal never required a finished rebuild. I sequenced it as if it did.
...
**Format adaptation isn't a product, and a payment rail isn't demand.** Both looked like wedges in April. Neither survived contact with how agents actually discover and pay.
