...
Pick a claim, open the file that implements it, then the tests that fail if it
stops being true.
...
|---|---|---|---|
| Canonical manifest line and root digest | §8.1 to §8.4 | `src/identity/canonical.ts` | `test/identity-core.test.ts`, `test/adversarial-serialization.test.ts` |
...
| Recorded-path form, rejected names, the ambiguity collision rule | §4.1 to §4.3 | `src/identity/walk.ts` (`validateRecordedPath`, name decoding), `src/identity/unicode-15-1.ts` | `test/adversarial-walker.test.ts`, `test/identity-core.test.ts` |
...
defective. The other documents explain: the README, the
...
is a way the tool could report a match, or exit clean, when it should not:
